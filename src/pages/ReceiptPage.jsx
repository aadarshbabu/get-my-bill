...
const ReceiptPage = () => {
  ...
  const handleItemChange = (index, field, value) => {
    ...
    if (field === "quantity" || field === "amount") {
      newItems[index].total = parseInt(newItems[index].quantity) * parseFloat(newItems[index].amount);
    }
    setItems(newItems);
  };
  ...
  const calculateSubTotal = () => {
    return items.reduce((sum, item) => parseFloat(sum) + parseFloat(item.total), 0).toFixed(2);
  };
